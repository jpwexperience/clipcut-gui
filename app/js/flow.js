...
function startMpv(mpvPath, film){
	var scriptPath = __dirname + '/../extraResources/stamps.lua';
	var filmPath = film.filepath;
	const mpvPlay = spawn(mpvPath, ['--osd-fractions', '--script=' + scriptPath, filmPath]);
	...
	mpvPlay.stdout.on('data', (data) => {
		getMpvEvent(data.toString(), film);
	});
...
//Gets MPV executable path and starts video playback
function playVid(id) {
	var tempFilm = findFilm(id);
	var mpvPath = "";
	var osPlatform = os.platform;
	var hasPathError = false;
	if (osPlatform == "win32"){
